...
def fetch_samaja_pages(d: date, edition_code: str, max_pages: int = 30):
    """Returns candidate (page_number, url) pairs to try — no downloading
    here, that happens later in parallel."""
    return [(page, samaja_page_url(d, edition_code, page)) for page in range(1, max_pages + 1)]
...
def fetch_sambad_pages(d: date, edition_code: str, max_pages: int = 30):
    """Returns candidate (page_number, url) pairs to try — no downloading here."""
    return [(page, sambad_page_url(d, edition_code, page)) for page in range(1, max_pages + 1)]
...
def fetch_dharitri_pages(d: date, edition_tuple, max_pages: int = 40):
    """Looks up the right edition id for this date (2 quick requests),
    then returns candidate (page_number, url) pairs — actual image
    downloads happen later, in parallel with everything else."""
...
MAX_OCR_WIDTH = 1100  # downscale before OCR; keeps keyword detection accurate, much faster

def process_page(paper, page_num, url, edition_choice, selected_date):
    """Downloads ONE page and OCRs it. Runs inside a worker thread so many
    pages download+read at once instead of one-at-a-time."""
    try:
        resp = requests.get(url, timeout=20)
    except requests.RequestException:
        return None
    if resp.status_code != 200 or len(resp.content) < 2000:
        return None  # page doesn't exist / edition doesn't go this far
...
def run_search(selected_papers, selected_date, edition_choice, progress_cb):
    """Looks up page URLs for each selected paper (fast, mostly sequential
    setup), then downloads + OCRs every page across every paper in ONE
    shared worker pool — this is what actually cuts wall-clock time, since
    dozens of page downloads now happen at once instead of one-by-one."""
    jobs = []  # (paper, page_num, url)
    for paper in selected_papers:
        ...
        if code is None:
            continue
        try:
            page_urls = info["fetch"](selected_date, code)
        except Exception:
            page_urls = []
        for page_num, url in page_urls:
            jobs.append((paper, page_num, url))
    ...
    with ThreadPoolExecutor(max_workers=12) as pool:
        future_map = {
            pool.submit(process_page, paper, page_num, url, edition_choice, selected_date): i
            for i, (paper, page_num, url) in enumerate(jobs)
        }
        done_count = 0
...
# UI
# ----------------------------------------------------------------------
st.set_page_config(page_title="ESS Tender Finder", page_icon="🛡️", layout="centered")
...
.ess-header img { height: 48px; border-radius: 6px; background: white; padding: 4px; }
.ess-header .ess-title { color: #ffffff; font-family: 'Poppins', sans-serif; font-weight: 700; font-size: 26px; line-height: 1.2; }
.ess-header .ess-sub { color: #D4A017; font-size: 13px; font-weight: 600; letter-spacing: 0.5px; }

div.stButton > button[kind="primary"], .stDownloadButton > button {
    background: #0B2545 !important; border: none !important; border-radius: 10px !important;
}
div.stButton > button[kind="primary"]:hover { background: #13315C !important; }
...
<div class="ess-header">
    <img src="https://executivesecurity.in/uploads/logo.png">
    <div>
        <div class="ess-title">Tender Finder</div>
        <div class="ess-sub">EXECUTIVE SECURITY SERVICES PVT LTD &nbsp;·&nbsp; SINCE 1996</div>
    </div>
</div>
